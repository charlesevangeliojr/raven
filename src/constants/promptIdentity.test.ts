...
test('CLI identity prefixes describe RAVEN instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('RAVEN')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('RAVEN')
    expect(prefix).not.toContain('Claude Code')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes RAVEN instead of Claude Code', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('RAVEN')
...
test('built-in agent prompts describe RAVEN instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('RAVEN')
  ...
  expect(generalPrompt).toContain('RAVEN')
  ...
  expect(explorePrompt).toContain('RAVEN')
  ...
  expect(guidePrompt).toContain('RAVEN')
  expect(guidePrompt).toContain('You are the RAVEN guide agent.')
  expect(guidePrompt).toContain('**RAVEN** (the CLI tool)')
